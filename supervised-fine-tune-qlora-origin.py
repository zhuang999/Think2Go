...
def train():
    ...
    # 明确设置为训练模式的attention，避免在训练过程中的生成阶段出现兼容性问题
    ...
    class CustomTrainer(Trainer):
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            # 为当前样本提取数据
            sample_input_ids = inputs["input_ids"][0].cpu().numpy()
            sample_labels = inputs["labels"][0].cpu().numpy()
            sample_predictions = torch.argmax(logits[0], dim=-1).cpu().numpy()
            ...
            prediction_text = tokenizer.decode(valid_predictions, skip_special_tokens=False)
