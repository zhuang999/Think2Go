...
class RecPOTrainer(GenRecTrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not wandb.run:
            ...
            # 从命令行参数获取wandb_name，如果没有则使用默认的auto_run_rank{rank}
            wandb_name = getattr(self.args, 'wandb_name', None)
            if wandb_name is None:
                wandb_name = f'auto_run_rank{rank}'
            else:
                # 如果提供了wandb_name，自动加上rank后缀以区分多进程
    ...
    #     """动态调整batch size的数据加载器"""
    #     # 根据当前epoch调整batch size
    #     current_epoch = self.state.epoch if self.state.epoch is not None else 0
    #     total_epochs = self.args.num_train_epochs
        
    #     if current_epoch > total_epochs / (3*2):
    #         # 0.5个epoch后，batch size改为2
    #         new_batch_size = 2
    #         if not hasattr(self, '_batch_size_logged_2') or not self._batch_size_logged_2:
    #             print(f"Epoch {current_epoch:.2f}: 切换到batch size 2")
    #             self._batch_size_logged_2 = True
    #         self.args.per_device_train_batch_size = new_batch_size
    #     else:
    #         # 前0.5个epoch，batch size为1
    #         new_batch_size = self.original_batch_size
    #         if not hasattr(self, '_batch_size_logged_1') or not self._batch_size_logged_1:
    #             print(f"Epoch {current_epoch:.2f}: 使用batch size {new_batch_size}")
    #             self._batch_size_logged_1 = True
    #         self.args.per_device_train_batch_size = new_batch_size
        
    #     # 重新计算mini_batch_size
    #     self.args.mini_batch_size = self.args.per_device_train_batch_size * self.args.generation_config.num_return_sequences
        
    #     # 打印当前配置
    #     if hasattr(self, '_batch_size_logged_2') and self._batch_size_logged_2:
    #         print(f"当前配置 - per_device_train_batch_size: {self.args.per_device_train_batch_size}, mini_batch_size: {self.args.mini_batch_size}")
    #     elif hasattr(self, '_batch_size_logged_1') and self._batch_size_logged_1:
    #         print(f"当前配置 - per_device_train_batch_size: {self.args.per_device_train_batch_size}, mini_batch_size: {self.args.mini_batch_size}")
    ...
    def compute_rec_score(self,
        ...
        # seq_labels_value = (seq_labels!=-100).float().sum(-1)
        # seq_input_ids_value = (seq_input_ids!= self.tokenizer.pad_token_id).float().sum(-1)
        # reward_neg = seq_labels_value*2 > seq_input_ids_value
        # rewards_length = torch.where(reward_neg, torch.tensor(-1.0), torch.tensor(0.0)).float()


        # # 使用列表推导式优化嵌套循环
        # acc, reward_soft_format, reward_strict_format = self.my_compute_metrics(seq_input_ids, seq_labels)

        # rewards = acc + reward_soft_format + reward_strict_format
        rewards = inputs['rewards'] #+ rewards_length
        acc = inputs['acc']
        # 判断rewards最后一个维度是否全为0或全相同
        # 假设rewards形状为[batch_size, num_samples]
        ...
        # 只对rewards全为0或全相同的行置零，其余行保持原有advantage
        ...
        for i, is_all_zero in enumerate(row_all_zero):
            if not is_all_zero:
                ...
                relabel_mask[i, 0] = True # 如果全是0，默认选择第0个
            # 如果全为0，relabel_mask[i] 保持全0
        
        # relabel_mask = advantage > 0

        # rich.print(f"[yellow]advantage_type: {advantage}[/yellow]")
    ...
    def training_step(self,
        ...
        try:
            with torch.no_grad():
                ...
                if current_epoch > 10.0:
                    ...
                    base_temperature = self.vllm_sampling_params.temperature  # 记录初始温度
                    temperature_step = 0.5  # 每次重采温度增加的步长
                    for i in range(batch_size):
                        for j in range(1):
                            ...
                            # 用accelerator同步：只要有一个进程需要重采，所有进程都重采
                            ...
                            if global_need_resample.item() > 0:
                                ...
                                if self.accelerator.is_main_process:
                                    print(f"[DEBUG] epoch {current_epoch} 第{i}个batch第{j+1}次重采")
                                torch.cuda.empty_cache()
                                inputs = self._generate_in_train(eval_model, inputs_origin)
                                # if j == 4 and self.accelerator.is_main_process:
                                #     print(f"[DEBUG] epoch {current_epoch} 连续5次采样")
                            else:
                                break
                    # 恢复初始温度，避免影响后续step
        ...
        # print(f"[DEBUG] ooms: {ooms}")
        # if (torch.is_tensor(ooms) and ooms.any().item()) or (not torch.is_tensor(ooms) and any(ooms)):
        ...
        for i in range(num_user_mini_batches):
            ...
            # # try:
            # print(f"[DEBUG] before backward, loss={loss.item()}")
            ...
            #     if 'lora' in n.lower():
            #         lora_params += 1
            #         print(f"[DEBUG] LoRA参数: {n}, requires_grad={p.requires_grad}")
            
            # print(f"[DEBUG] 总参数: {total_params}, 可训练参数: {trainable_params}, LoRA参数: {lora_params}")
    ...
    def batch_forward(self, model, batch):
        ...
        # # 对最后一维的entropy进行排序，设置mask矩阵（前20%为1，其余为0）
        # # entropy: [batch_size, seq_len] 或 [batch_size, ...]
        # # 将非loss_mask位置的熵值设为极小值，确保它们不会被选中
        # masked_entropy = entropy.clone()
        # masked_entropy[~loss_mask] = float('-inf')

        # # 计算每个样本在loss_mask=1位置中需要选择的数量（前20%）
        # loss_mask_sums = loss_mask.sum(-1)  # [batch_size] - 每个样本中loss_mask=1的数量
        # k_values = torch.clamp((loss_mask_sums * 0.2).long(), min=1)  # 至少选择1个

        # # 对masked_entropy进行排序，获取排序后的索引
        # _, indices = torch.sort(masked_entropy, dim=-1, descending=True)

        # # 创建排序位置的mask：标识排序后的前k个位置
        # batch_size, seq_len = masked_entropy.shape
        # ranks = torch.arange(seq_len, device=entropy.device).unsqueeze(0).expand(batch_size, -1)  # [batch_size, seq_len]
        # topk_positions = ranks < k_values.unsqueeze(1)  # [batch_size, seq_len] - 标识排序后的前k个位置

        # # 创建最终mask：将排序后的前k个位置映射回原始位置
        # final_mask = torch.zeros_like(entropy, dtype=torch.bool)
        # final_mask.scatter_(-1, indices, topk_positions)

        # # 确保最终结果只在loss_mask=1的位置中
        # final_mask = final_mask & loss_mask

        # per_token_logps = per_token_logps * final_mask.float()
        ...
        # alpha_entropy = alpha * entropy.detach()
        # abs_adv_kappa = torch.abs(advantages)/kappa
        # psi_Ht = torch.min(alpha_entropy, abs_adv_kappa)
        # # 如果psi_Ht由entropy决定（即psi_Ht==alpha_entropy），则打印
        # entropy_min_mask = (psi_Ht == alpha_entropy)
        # if torch.any(entropy_min_mask):
        #     print("[DEBUG] psi_Ht由entropy决定的样本:")
        #     print("alpha * entropy.detach():", alpha_entropy[entropy_min_mask])
        #     print("torch.abs(advantages)/kappa:", abs_adv_kappa[entropy_min_mask])
        #     print("psi_Ht:", psi_Ht[entropy_min_mask])
        
        # print("entropy:", alpha * entropy.detach())
        # print("psi_Ht_before:", torch.abs(advantages)/kappa)
        # print("psi_Ht:", psi_Ht)
        shaped_advantages = advantages + psi_Ht
        # gamma = 0.2
        # shaped_advantages = (1 + gamma * norm_rewards) * shaped_advantages
        norm_rewards = 1 - ((4.5 - rewards) / 4.5)
        gamma = 1.0
        #gamma = gamma * (1-time_difficulties) + gamma * (1-spatial_difficulties)

        # case 1: 
        # gamma = gamma * (1-torch.min(time_difficulties, spatial_difficulties))
        ...
        # a = 0.55
        # b=0.15
        # gamma = -(((0.5 * time_difficulties + 0.5 * spatial_difficulties) - a)**2) / (2 * b**2)


        shaped_advantages = torch.exp(norm_rewards) * shaped_advantages
