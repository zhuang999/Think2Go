...
class GenRecTrainer(Trainer):
            ...
            if self.accelerator.is_main_process:
                ...
                with world_size_patch, profiling_patch:
                    ...
                    self.llm = LLM(
                        ...
                        # quantization="bitsandbytes", # 指示vLLM进行即时量化
                    ...
                    # 通过猴子补丁修改lm_head的forward方法，确保输出为float32，同时保持所有原始属性
                    ...
                    print("[LOG][vLLM] step2.5: lm_head的forward方法已修改为输出float32")

            print("[LOG][vLLM] step2: vLLM LLM 对象初始化完成")
    ...
    def _move_model_to_vllm(self):
        ...
        # # 如果没有保存初始参数，则保存当前参数作为基准
        # if not hasattr(self, '_initial_params'):
        #     print("📌 首次调用，保存当前参数作为基准...")
        ...
        #             self._initial_params[name] = param.clone().detach().cpu()
        #     print(f"✅ 已保存 {len(self._initial_params)} 个参数作为基准")
            
        #     # 首次调用时，没有变化的参数
        #     changed_params = []
        #     unchanged_params = list(self._initial_params.keys())
        # else:
        #     # 对比参数变化
        #     print("🔍 对比参数变化中...")
        #     changed_params = []
        ...
        #     with unwrap_model_for_generation(self.model, self.accelerator, gather_deepspeed3_params=True) as unwrapped_model:
        #         for name, param in unwrapped_model.named_parameters():
        #             if name in self._initial_params:
        #                 initial_param = self._initial_params[name].to(param.device)
        #                 # 计算参数差异
        #                 diff = torch.abs(param - initial_param).max().item()
        #                 if diff > 1e-8:  # 设置一个小的阈值来判断是否有变化
        ...
        # # 打印结果
        # print(f"\n📊 参数变化统计:")
        # print(f"   总参数数: {len(self._initial_params)}")
        # print(f"   已变化参数数: {len(changed_params)}")
        # print(f"   未变化参数数: {len(unchanged_params)}")
        
        # if changed_params:
        #     print(f"\n✅ 发生变化的参数 ({len(changed_params)}个):")
        #     # 按照变化幅度排序
        #     changed_params.sort(key=lambda x: x['max_diff'], reverse=True)
        #     for i, param_info in enumerate(changed_params[:20]):  # 只显示前20个变化最大的
        #         print(f"   {i+1:2d}. {param_info['name']}")
        #         print(f"       shape: {param_info['shape']}, max_diff: {param_info['max_diff']:.6e}")
            
        #     if len(changed_params) > 20:
        #         print(f"   ... 还有 {len(changed_params) - 20} 个参数发生了变化")
            
        #     # 统计变化参数的类型
        #     param_types = {}
        #     for param_info in changed_params:
        #         name = param_info['name']
        #         if 'lora' in name.lower():
        #             param_types['LoRA'] = param_types.get('LoRA', 0) + 1
        #         elif 'embed' in name:
        #             param_types['Embedding'] = param_types.get('Embedding', 0) + 1
        #         elif 'lm_head' in name:
        #             param_types['LM_Head'] = param_types.get('LM_Head', 0) + 1
        #         elif 'norm' in name:
        #             param_types['Norm'] = param_types.get('Norm', 0) + 1
        #         elif any(x in name for x in ['q_proj', 'k_proj', 'v_proj', 'o_proj']):
        #             param_types['QKV_Proj'] = param_types.get('QKV_Proj', 0) + 1
        #         else:
        #             param_types['Other'] = param_types.get('Other', 0) + 1
        ...
        with unwrap_model_for_generation(
                    ...
                    if name.endswith("base_layer.weight"):
                        ...
                        if a_name in param_dict and b_name in param_dict:
                            ...
                            if quant_state is not None:
                                ...
                                # print(f"lora_b: {lora_b}")
                                # print("lora_a max:", lora_a.abs().max())
                                # print("lora_b max:", lora_b.abs().max())
                                # print(f"scaling: {scaling}")
                                # print(f"lora_delta: {lora_delta}")
                                merged_fp = base_fp + lora_delta.to(base_fp.device).to(base_fp.dtype)

                                # --- 您要求的比较逻辑 开始 ---
                                # 1. 比较合并前后的浮点权重 (base_fp vs merged_fp)
                                # are_equal_after_merge = torch.equal(merged_fp, base_fp)
                                # print(f"📊 LoRA 合并效果: 是否与合并前完全相同? {are_equal_after_merge}")

                                # # 2. 比较经过"量化->解量化"完整流程后的权重和原始基础权重
                                # # 重新量化 merged_fp
                                # merged_q, new_quant_state = F.quantize_4bit(merged_fp, quant_type=quant_state.quant_type, blocksize=quant_state.blocksize)
                                # 将其再解量化回来用于比较
                                
                                # are_equal_after_roundtrip = torch.equal(param, merged_q)
                                # print(f"📊 完整流程差异 (param vs merged_q): 是否与合并前完全相同? {are_equal_after_roundtrip}")
                                # --- 您要求的比较逻辑 结束 ---
            ...
            if self.accelerator.is_main_process:
                ...
                #     state_dict_on_target_device[name] = weight.to(target_device)
                # state_dict = state_dict_on_target_device
                ...
                # # 🔍 循环 vLLM 模型权重，对比训练模型权重
                # print("🔍 开始对比 vLLM 权重与训练模型权重...")
                # for name, param in llm_model.named_parameters():
                #     if name in state_dict:
                #         # 保存统计信息和完整的权重张量
                #         params_stats_before[name] = {
                #             'mean': param.mean().item(),
                #             'std': param.std().item(),
                #             'max': param.max().item(),
                #             'min': param.min().item(),
                #             'shape': param.shape,
                #             'tensor': param.clone().detach().cpu()  # 保存完整的权重张量
                #         }
                        
                #         # 跳过 embedding 和 lm_head 权重的对比
                #         if 'embed' in name or 'lm_head' in name:
                #             continue
                        
                #         # 直接对比训练模型权重与vLLM权重
                #         are_equal = torch.equal(state_dict[name].to(param.device), param)
                #         max_diff = torch.abs(state_dict[name].to(param.device) - param).max().item()
                ...
                #         if not are_equal:
                #             print(f"      ⚠️  训练模型权重与vLLM权重不匹配！")
                #         else:
                #             print(f"      ✅ 训练模型权重与vLLM权重完全匹配")
                #     else:
                #         print(f"  ❌ state_dict中未找到对应权重: {name}")
                
                # print(f"\n准备对比 {len(params_stats_before)} 个参数的变化...")
                # # --- END DEBUG BEFORE LOADING ---
                ...
                # # 对比加载前后的参数变化
                # for name, stats_before in params_stats_before.items():
                #     # 获取加载后的参数
                #     param_after = dict(llm_model.named_parameters())[name]
                    
                #     # 🔧 修复：直接比较权重张量是否相等
                #     # 获取加载前保存的权重张量
                #     param_before = stats_before['tensor']  # 之前保存的完整张量
                    
                #     # 直接比较权重张量是否相等
                #     weights_equal = torch.equal(param_before.cpu(), param_after.cpu())
                ...
                #         # 计算权重差异的统计信息用于显示
                #         weight_diff = torch.abs(param_before.cpu() - param_after.cpu())
                        
                #         print(f"  🔄 变化: {name}")
                
                # print(f"\n📊 总结:")
                # print(f"  ✅ 发生变化的参数: {len(changed_params)}")
                # print(f"  ❌ 未发生变化的参数: {len(unchanged_params)}")
                
                # # if unchanged_params:
                # #     print(f"\n⚠️  未变化的参数列表:")
                # #     for name in sorted(unchanged_params):
                # #         print(f"      {name}")
                
                # print("=" * 50)
                # # --- END DEBUG AFTER LOADING ---
