...
class TrainingArguments(RecPOTrainingArguments):
    ...
    # Generation config parameters (避免JSON序列化问题)
    ...
    def to_dict(self):
        ...
        d = super().to_dict()
        # 移除generation_config，因为它不能被JSON序列化
...
def smart_tokenizer_and_embedding_resize(
    ...
    if num_new_tokens > 0:
        ...
        # 新增：用旧tokenizer分解新token，embedding用分解token的加权和
        # 假设 special_tokens_dict['additional_special_tokens'] 是新加的token列表
        # 假设 old_tokenizer 变量可用，且未包含新token
        # for i, token in enumerate(special_tokens_dict.get('additional_special_tokens', [])):
        #     new_token_id = tokenizer.convert_tokens_to_ids(token)
        #     split_token_ids = old_tokenizer(token, add_special_tokens=False)["input_ids"]
        #     split_token_ids = [tid for tid in split_token_ids if tid < input_embeddings.shape[0] - num_new_tokens]
        #     if len(split_token_ids) == 0:
        #         new_embed = input_embeddings[:-num_new_tokens].mean(dim=0)
        #         new_out_embed = output_embeddings[:-num_new_tokens].mean(dim=0)
        #     else:
        #         new_embed = input_embeddings[split_token_ids].mean(dim=0)
        #         new_out_embed = output_embeddings[split_token_ids].mean(dim=0)
        #     input_embeddings[new_token_id] = new_embed
        #     output_embeddings[new_token_id] = new_out_embed

    # print("新增的特殊token及其token id：")
...
def _tokenize_fn(strings: Sequence[str], tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    tokenized_list = [
        tokenizer(
            ...
            truncation=True,
            add_special_tokens=False,  # 不自动添加特殊token如<begin_of_text>
        )
        for text in strings
    ]
...
def preprocess(
    ...
    # sources是用户部分，targets是assistant回复部分
    # 完整序列 = sources + targets
    ...
    # 根据sources的长度设置IGNORE_INDEX（只有targets部分参与损失计算）
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        for example in list_data_dict:
            ...
            sources.append(history_part)  # 历史数据部分，将被设为IGNORE_INDEX
            targets.append(reasoning_part)  # 推理和预测部分，参与损失计算
        ...
        #         print(f"样本 {i+1}:")
        #         print(f"Source (历史数据部分，将被IGNORE):\n{sources[i]}")
        #         print(f"Target (推理和预测部分，参与损失计算):\n{targets[i]}")
        #         print(f"完整序列 (历史数据 + 推理预测):\n{sources[i] + targets[i]}")
        #         print("=" * 50)
        #     print("========================\n")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def torch_call(self, examples):
        ...
        for i in range(examples["input_ids"].shape[0]):
            ...
            for j in range(len(reasoning)):
                ...
                #     truncation=True,
                # )
                #batch["gen_attention_mask"].append(conversation["attention_mask"])
                # 从 user_content 之后开始搜索，以跳过 prompt 中的示例

                # prediction_id, result = self.find_completion_start_end_special_token(conversation.tolist(), search_start_index=user_content_len)
                # 使用列表推导式优化嵌套循环
                ...
                # 如果最后一个token是eos_token_id，先去掉
                if conversation[-1].item() == self.tokenizer.eos_token_id:
                    conversation = conversation[:-1]
                    reasoning_len -= 1
                conversation_reasoning_len = conversation.shape[0]
                # print("[debug] transition_prompt", transition_prompt)
                # print("[debug] error_miss_token_prompt", error_miss_token_prompt)
                # print("[debug] miss_token", miss_token)
                # print("[debug] token_id", token_id)
                # if miss_token.shape[0] > 0:
                #     conversation = torch.cat([conversation, transition_prompt, error_miss_token_prompt, miss_token, token_id], dim=0)
                # else:
                #     conversation = torch.cat([conversation, transition_prompt, right_miss_token_prompt, miss_token, token_id], dim=0)

                # if id_error.shape[0] > 0:
                #     conversation = torch.cat([conversation, error_id_error_prompt, id_error, token_id], dim=0)
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # input_ids_pad = torch.nn.utils.rnn.pad_sequence(input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
    ...
    def find_completion_start_end_special_token(self, current_input_ids, search_start_index=0):
        """
        先查找<answer>，再查找</answer>，返回其后的内容位置
        """
    ...
    def my_compute_metrics(self, predictions, ground_truths):
        ...
        import re
        miss_token = []
        format_error = 0  # 0: 格式错误，1: 格式正确，2: 格式正确但语义ID错误
        id_error = []
        pred_answer_text = ''
        

        acc = 0  # 先全0，后面有有效样本再赋值为1
        ...
        # if len(pred_text) < 300:
        #     rep_penalty += 1.0
        
        # # 检查是否只输出了<|end_of_text|>，严重惩罚这种逃避行为
        # if pred_text.strip() == '<|end_of_text|>' or pred_text.strip() == '':
        #     reward_soft_format -= 1.0

        # 软奖励：<think>、<answer>:、<|end_of_text|>都只出现一次
        # for text in ['</think>', '</answer>', '<|end_of_text|>']:
        #     if pred_text.count(text) == 1 and len(pred_text) > 100:
        #         reward_soft_format += 0.2
            # elif pred_text.count(text) == 0:
            #     miss_token.extend(self.tokenizer.encode(text, add_special_tokens=False))
            # elif pred_text.count(text) > 1:
            #     id_error.extend(self.tokenizer.encode(text, add_special_tokens=False))

        if pred_text.count('<answer>:') == 1 and len(pred_text) > 300:
            # 检查<answer>:后面是否有有效内容，而不是空答案
