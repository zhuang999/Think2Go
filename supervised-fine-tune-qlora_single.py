...
def extract_semantic_sequences(text):
    ...
    # 将提取的语义ID转换为标准格式
    semantic_ids = []
    for type_char, value in matches:
        semantic_ids.append(f"<{type_char}_{value}>")
...
def smart_tokenizer_and_embedding_resize(
    ...
    if data_path:
        ...
        if all_ids:
            ...
            print(f"添加 {len(semantic_tokens)} 个语义ID到词汇表")
            num_semantic_tokens = tokenizer.add_special_tokens(semantic_tokens)
...
def preprocess(
    ...
    if len(sources) > 0 and len(targets) > 0:
        ...
        print(f"Source中的语义ID: {source_ids}")
        print(f"Target中的语义ID: {target_ids}")
        print("===========================\n")
